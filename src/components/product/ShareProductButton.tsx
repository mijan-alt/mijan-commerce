...
export function ShareProductButton({ product }: { product: Product }) {
  ...
  const shareData = {
    title: product.title,
    text: `Check out ${product.title} - ₦${product.priceInNGN?.toLocaleString()}`,
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div
          ref={dropdownRef}
          className="absolute right-0 top-full mt-2 w-56 rounded-lg border bg-popover shadow-lg z-50 animate-in fade-in-0 zoom-in-95"
        >
          <div className="p-1">
            {/* Copy Link */}
            <button
